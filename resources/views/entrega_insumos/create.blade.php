...
@section('content_header')
    <h1 class="m-0 custom-heading">{{ __('Nueva Entrega de Insumos') }}</h1>
@stop
...
@section('content')
    <form action="{{ route('entrega_insumo.store') }}" method="post" autocomplete="off">
        @csrf
        @method('POST')
            ...
            <div class="card-body">
                <div class="row mb-3">
                    <x-adminlte-input type="date" id="fecha" name="fecha" label="Fecha"
                        value="{{ date('Y-m-d') }}" fgroup-class="col-md-3" required />
                    <x-adminlte-select2 name="id_persona" id="id_persona" label="Persona Receptora"
                        ...
                        @foreach ($personas as $item)
                            <option value="{{ $item->id }}">{{ $item->nombre }} {{ $item->apellido }} ({{ $item->documento }})</option>
                        @endforeach
                    </x-adminlte-select2>
                </div>
                ...
                <div class="row">
                    <div class="col-md-12 text-right">
                        ...
                        <x-adminlte-button type="submit" label="Registrar Entrega" theme="primary" icon="fas fa-lg fa-save" />
                    </div>
                </div>
            </div>
        </div>
