...
@section('content_header')
    <h1 class="m-0 text-dark">{{ __('Actualizar Evento') }}</h1>
@stop
            ...
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('evento.update', $evento) }}" method="post">
                        ...
                        <div class="row">
                            <x-adminlte-select name="tipo" label="Tipo" data-placeholder="{{ __('Seleccionar una opción...') }}"  fgroup-class="col-md-6">
                                ...
                                @if ($evento->tipo === 0)
                                    <option selected value="0">{{ __('Seleccione') }}</option>
                                    <option value="1">{{ __('Concierto') }}</option>
                                    <option value="2">{{ __('Fútbol') }}</option>
                                @elseif ($evento->tipo === 1)
                                    <option value="0">{{ __('Seleccione') }}</option>
                                    <option selected value="1">{{ __('Concierto') }}</option>
                                    <option value="2">{{ __('Fútbol') }}</option>
                                @else
                                    <option value="0">{{ __('Seleccione') }}</option>
                                    <option value="1">{{ __('Concierto') }}</option>
                                    <option selected value="2">{{ __('Fútbol') }}</option>
                                @endif
                            </x-adminlte-select>
                        </div>
                        <div class="row">
                            <div class="form-group col-md-6">
                                <div class="pull-right">
                                    <a href="" class="btn btn-default btn-flat">{{ __('Cancelar') }}</a>
                                </div>
