...
<h1 class="m-0 custom-heading">{{ __('Editar Datos Del Cliente') }}</h1>
@stop
            ...
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('cliente.update', $cliente = $cli) }}" method="post" autocomplete="off">
                        ...
                        <div class="row">
                            <x-adminlte-select name="estado" label="Estado del Cliente" data-placeholder="{{ __('Seleccionar una opción...') }}" fgroup-class="col-md-3">
                                <option value="1" {{ $cliente->estado == 1 ? 'selected' : '' }} >{{ __('Activo') }}</option>
                                <option value="0" {{ $cliente->estado == 0 ? 'selected' : '' }} >{{ __('Inactivo') }}</option>
                            </x-adminlte-select>
                        </div>
