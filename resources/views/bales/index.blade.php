...
<td>@if($bale->web_image_url)<img src="{{ $bale->web_image_url }}" alt="{{ $bale->code }}" class="img-thumbnail" style="width:72px;height:72px;object-fit:cover">@else<span class="text-muted">Sin foto</span>@endif</td><td><strong>{{ $bale->code }}</strong></td><td>{{ $bale->purchase_date->format('d/m/Y') }}</td><td>{{ $bale->bale_type ?: '—' }}<br><small>{{ $bale->supplier_name }}</small></td>
