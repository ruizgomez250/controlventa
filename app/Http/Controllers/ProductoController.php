...
class ProductoController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // Si viene marcado desde el formulario, guarda 1.
            // Si no viene, guarda 0.
            $estado = $request->input('estado', null);
            $estado = $estado !== null ? ($estado ? "1" : "0") : "0";
            ...
            // Validación principal del producto.
            $validated = $request->validate([
                ...
                'pcosto'        => 'required|numeric|min:0',
                'pventa'        => 'required|numeric|min:0',
                'estado'        => 'required|in:0,1',
                'imagen'        => 'nullable|image|mimes:jpg,jpeg,png|max:2048',
            ]);

            // Se obtiene el valor del impuesto seleccionado en el formulario.
            // La tabla productos tiene dos campos relacionados al impuesto:
            // - id_impuesto: guarda la referencia al registro de la tabla impuestos.
            // - impuesto: guarda el valor numérico del impuesto, por ejemplo 10, 5 o 0.
            // Si no se completa el campo impuesto, MySQL genera el error:
            // Field 'impuesto' doesn't have a default value.
            $impuestoValor = Impuesto::where('id', $request->id_impuesto)->value('valor');

            // Se agrega el valor del impuesto al array validado,
            // para que el INSERT incluya también la columna obligatoria impuesto.
            $validated['impuesto'] = $impuestoValor ?? 0;

            // Valor por defecto para stock si no viene desde el formulario.
            $validated['stock'] = $validated['stock'] ?? 0;

            // Si se carga imagen, se guarda en storage/app/public/productos.
            ...
            // Guarda los tramos de precio mayorista, si vienen desde el formulario.
            if ($request->has('tier_cantidad')) {
                foreach ($request->tier_cantidad as $i => $cantidad) {
                    if (isset($request->tier_precio[$i]) && $cantidad > 0) {
                        ProductoPrecioTier::create([
